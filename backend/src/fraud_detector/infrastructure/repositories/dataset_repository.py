...
COPY_STATEMENT = (
    "COPY transacao (id_dataset, indice_origem, valor, tempo_relativo, atributos, rotulo_real) "
    "FROM STDIN"
)
...
class RepositorioDataset:
    ...
    def create_with_copy(
        ...
        *,
        ...
        try:
            ...
            with driver.cursor() as cursor:
                with cursor.copy(COPY_STATEMENT) as copy:
                    ...
                    for line in parser.iter_copy_text(dataset.id_dataset):
                        ...
                        if on_progress and total_records % progress_every_rows == 0:
                            on_progress(dataset.id_dataset, total_records)
                        parse_started = perf_counter()
                    parse_seconds += perf_counter() - parse_started
                    if buffer:
                        write_started = perf_counter()
            ...
            if total_records == 0:
                raise ValueError("O arquivo CSV precisa conter ao menos uma transação")
    ...
    def list_by_owner(self, owner_id: int, offset: int, limit: int) -> list[DatasetModel]:
        return (
            ...
            .order_by(DatasetModel.importado_em.desc(), DatasetModel.id_dataset.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )

    def get_by_id(self, dataset_id: int, owner_id: int) -> DatasetModel | None:
        return (
            self.db.query(DatasetModel)
            .filter(
    ...
    def delete_owned(self, dataset_id: int, owner_id: int) -> str | None:
        try:
            result = self.db.execute(
                text(
                    "DELETE FROM dataset "
                    "WHERE id_dataset = :dataset_id AND id_usuario = :owner_id "
